...
class ByteUtilTest {
    ...
    void check_hash_with_same_key() {
        ...
        System.out.println(for_test.hash(A));
        boolean answer = for_test.hash(A)==for_test.hash(A);
    ...
    void check_hash_with_different_key() {
        ...
        assertEquals(false, for_test.hash(A)==for_test.hash(B));
    }
    ...
    void check_proto(){
        ...
        long val2 = 123342431;
        long val3 = 867456353;
        byte[] ans= Epidemic.EpidemicInfo.newBuilder()
                .addTimeTag(val1)
                .addTimeTag(val2)
                .build().toByteArray();
        ...
        try {
            long get_val1=Epidemic.EpidemicInfo.PARSER.parseFrom(ans, 0, ans.length).getTimeTag(0);
            System.out.println(get_val1);

            long get_val2=Epidemic.EpidemicInfo.PARSER.parseFrom(ans, 0, ans.length).getTimeTag(1);
